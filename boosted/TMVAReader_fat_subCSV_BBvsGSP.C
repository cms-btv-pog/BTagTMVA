...
void TMVAReader(){
    ...
    reader->AddVariable("SubJet1_CSVIVF", &SubJet1_CSVIVF);
    reader->AddVariable("SubJet2_CSVIVF", &SubJet2_CSVIVF);
    ...
    reader->BookMVA( "BDTG_T1000D3_fat_subCSV_BBvsGSP method", "weights/TMVATrainer_BDTG_T1000D3_fat_subCSV_BBvsGSP.weights.xml" );
    ...
    for(Long64_t iEntry = 0; iEntry < intree->GetEntries(); iEntry++){
	    ...
	    bool isBkg = ( abs(int(Jet_flavour))==5 && Jet_nbHadrons>1 && Jet_massGroomed>80 && Jet_massGroomed<150 );
	    float BDTG_Disc = reader->EvaluateMVA("BDTG_T1000D3_fat_subCSV_BBvsGSP method");
    ...
    TString outname = "BDTG_vs_CSVv2IVF_fat_subCSV_BBvsGSP.root";
