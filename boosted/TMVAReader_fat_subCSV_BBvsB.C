...
void TMVAReader(){
    ...
    reader->BookMVA( "BDTG_T1000D3_fat_subCSV_BBvsB method", "weights/TMVATrainer_BDTG_T1000D3_fat_subCSV_BBvsB.weights.xml" );
    ...
    for(Long64_t iEntry = 0; iEntry < intree->GetEntries(); iEntry++){
	    ...
	    bool isBkg = ( abs(int(Jet_flavour))==5 && Jet_nbHadrons<2 && Jet_massGroomed>80 && Jet_massGroomed<150 );
	    float BDTG_Disc = reader->EvaluateMVA("BDTG_T1000D3_fat_subCSV_BBvsB method");
    ...
    TString outname = "BDTG_vs_CSVv2IVF_fat_subCSV_BBvsB.root";
