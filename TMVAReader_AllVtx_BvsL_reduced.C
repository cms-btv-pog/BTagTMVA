...
void TMVAReader(int vtxCategory=-1){
    ...
    reader->BookMVA( "BDTG_T1000D3_BvsL_reduced method", "weights/TMVATrainer_AllVtx_BDTG_T1000D3_BvsL_reduced.weights.xml" );
    ...
    for(Long64_t iEntry = 0; iEntry < intree->GetEntries(); iEntry++){
	    ...
	    float BDTG_Disc = reader->EvaluateMVA("BDTG_T1000D3_BvsL_reduced method");
    ...
    if( vtxCategory==0 )
      postfix = "RecoVtx_AllVtxTraining";
    else if( vtxCategory==1 )
      postfix = "PseudoVtx_AllVtxTraining";
    else if( vtxCategory==2 )
      postfix = "NoVtx_AllVtxTraining";
    outname += postfix;
    outname += "_BvsL_reduced.root";
